...
class Neuraltrain_1_1(object):
    ...
    def __update(self, dW, dB, epoch):
        if epoch > 1000:
            self.W = self.W - self.hp.eta * dW / np.sqrt(epoch+1)
            self.B = self.B - self.hp.eta * dB / np.sqrt(epoch+1)
        else :
    ...
    def train(self, dataReader, checkpoint=0.1):
        ...
        for epoch in range(self.hp.max_epoch):
            # print("epoch=%d" %epoch)
            dataReader.Disorganize()
            for iteration in range(max_iteration):
                batch_x, batch_y = dataReader.GetTrainSamples(self.hp.batch_size, iteration)
                batch_z = self.__forwardBatch(batch_x)
                dW, dB = self.__backwardBatch(batch_x, batch_y, batch_z)
                self.__update(dW, dB, epoch)
